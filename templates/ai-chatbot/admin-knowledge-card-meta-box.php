...
<div class="ai-knowledge-meta">
    <div class="ai-chatbot-field-row">
        <div class="ai-chatbot-field"><label for="knowledge_metadata_provider_id"><?php esc_html_e('Metadata Provider', 'wp-aigent'); ?></label><select id="knowledge_metadata_provider_id" name="knowledge_metadata_provider_id"><option value="0"><?php esc_html_e('— Disabled —', 'wp-aigent'); ?></option><?php foreach ($providers as $provider): ?><option value="<?php echo esc_attr($provider->ID); ?>" <?php selected($metadata_provider_id, $provider->ID); ?>><?php echo esc_html($provider->post_title); ?></option><?php endforeach; ?></select></div>
    ...
    <p class="description"><?php printf(esc_html__('Status: %1$s · Indexed chunks: %2$d', 'wp-aigent'), esc_html($card['status'] ?: 'stale'), count($chunks)); ?></p>
</div>
